...
the algebraic factorization-centre lift available after passing to
...
principal-part model carries a strict algebraic \(P_0\)
factorization-centre lift with zero homotopies; the ordinary
scalar-reduced trace model has a nonzero central CE class; the
unreduced central-character lift and the balanced supertrace model
cancel that scalar class exactly.
...
\begin{constr}[Algebraic factorization-centre lift]
...
\begin{thm}[Algebraic current factorization-centre lift with zero homotopies]
\label{thm:app-algebraic-factorization-centre-zero-homotopies}
  ...
  factorization-centre lift as far as the distributional algebraic
  current model permits.  It does not contradict the polynomial no-go
...
\begin{proof}
  ...
  Definition~\ref{def:app-central-character-principal-part-current-algebra}.
  Hence the CE differential and the target Schouten/\(P_0\)
  differential agree on generators, and the biderivation property
  extends the equality to the completed symmetric algebra.
  ...
  The same statement holds against arbitrary open observables in the
  completed algebra, not only against length-one generators.  Let
  ...
  open observables are strict zero homotopies in this current model.
\end{proof}
...
\label{prop:app-scalar-contact-qme-class}
  ...
  not exact in the scalar-reduced Hamiltonian source.  Hence the
...
\begin{proof}
  ...
  the only scalar primitive.  It lives on the unreduced polynomial
  Hamiltonian algebra and does not descend to \(\bar A\). Thus the
  scalar-reduced source has no counterterm whose associated graded
  kills the class.
\end{proof}

\begin{thm}[Central-character cancellation in the unreduced source]
\label{thm:app-central-character-qme-cancellation}
  ...
  \]
  Thus the scalar QME obstruction
  \(\operatorname{Ob}_{\mathrm{sc}}\) vanishes after replacing the
  scalar-reduced source by the unreduced source with central character
  \(\chi_N\).  Equivalently, the central character is the boundary
  realization of the unreduced scalar primitive of
  Lemma~\ref{lem:app-unreduced-scalar-primitive}, multiplied by \(N\)
  and by the Weyl factor \(\hbar\) in the quantum normalization.
\end{thm}
...
\begin{proof}
  ...
  \]
  By Theorem~\ref{prop:app-scalar-contact-qme-class},
  \[
...
\begin{thm}[Balanced supertrace QME cancellation]
...
\begin{proof}
  ...
  Theorem~\ref{prop:app-scalar-contact-qme-class} therefore gives the
...
\label{thm:app-ordinary-glN-scalar-obstruction-criterion}
  ...
  graded is \(w_{\mathrm{ext}}\in C^2_{\mathrm{CE}}(\bar A;\C)\).  A
  cancellation
  \[
  ...
  \[
    [w_{\mathrm{ext}}]=-\hbar N[\bar c]
      \qquad\text{in }H^2_{\mathrm{CE}}(\bar A;\C).
  \]
...
\begin{proof}
  ...
  \[
    \hbar N\omega+w_{\mathrm{ext}}+\delta\bar\eta=0
      \qquad\text{in }C^2_{\mathrm{CE}}(\bar A;\C),
  \]
  ...
  \(\hbar N[\bar c]=0\).  For \(N>0\) and formal \(\hbar\), this
  contradicts Theorem~\ref{prop:app-scalar-contact-qme-class}, which
