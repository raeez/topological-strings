...
\begin{thm}[Mixed-Dunn universal acceptance criterion]\label{thm:mixed-dunn-criterion}
  ...
  in Definition~\ref{defn:mixed-dunn-fact6r-datum}.  The mixed-Dunn
...
\begin{proof}
  ...
  Conversely, suppose (ii) holds.  The cofibrant filtration reduces
  rectification of \(\delta_{m,n}\) to a tower of lifting problems.  The
...
\begin{defn}[Named target convention data]\label{rmk:matched-conventions-templates}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Vol III CY-to-chiral datum.}
      ...
      \mathrm{Sp}_{\Sigma_{d-1},C},\kappa_{\mathrm{ch}},
      ...
      \(\kappa_{\mathrm{ch}}\) is the Hodge supertrace
      \(\sum_q(-1)^qh^{0,q}(Y)\); in particular
      \(\kappa_{\mathrm{ch}}(K3\times E)=0\).  Any BKM weight comparison
      is therefore a separate component of
