...
cohomology, not the full standard \v{C}ech lattice; the full principal-part
...
\begin{proof}
  ...
  \(c_{a,b+1}=c_{a,b}\) whenever both coefficients occur in the reduced
  index set, because
  \(z_1\cdot\rho_{a,b}=-(b+1)\rho_{a,b+1}\) and
  \(\{z_1,z_1^m z_2^n\}=n z_1^m z_2^{n-1}\).  Equivariance for
  \(z_2\) gives \(c_{a+1,b}=c_{a,b}\) under the same reduced-index
  condition.  The omitted \(c_{0,0}\) is not a coefficient of
  \(\mathcal P\).  Connecting through \(c_{1,1}\), all coefficients with
