...
coadjoint representation.  The Fourier--Rees comparison constructed
below is only a Weyl-module and associated-graded label comparison.
Over \(\C[[\hbar]]\) its target is the Fourier delta Rees lattice
inside local cohomology, not the full standard Cech lattice; the full
principal-part lattice appears only after tensoring with
\(\C((\hbar))\).  It does not identify \(\Psi_{a,b}\) with
\(\rho_{a,b}\) as \(\mathfrak h\)-modules.  The source conventions are
Matlis duality
...
\label{def:app-matlis-topology}
  ...
  \]
  under this section.  The Lie bracket is the Poisson bracket
  \[
...
\label{prop:app-matlis-realization}
  ...
  \(\mc P=\operatorname{Ann}_{\mathrm{res}}(\C\cdot1)\) is not asserted to be an
  \(R\)-submodule, and \(R/\C\cdot1\) is not an \(R\)-module quotient;
  it is the \(\C\)-linear scalar-annihilator dual to
  \(\mathfrak h=R/\C\cdot1\) and is stable for the Hamiltonian
  coadjoint action below.
\end{prop}
...
\begin{thm}[No polynomial descendant realization]
\label{thm:app-matlis-no-polynomial-realization}
...
\begin{thm}[Fourier--Rees bridge, and the obstruction it does not remove]
\label{thm:app-matlis-rees-fourier-bridge}
  ...
  \(\Delta^{\mathsf F}_\hbar\).  On basis labels in the Cech
  realization,
  \[
    z_1^az_2^b
    \longmapsto
    ...
    (-1)^{a+b}\hbar^{a+b}a!b!\,\rho_{a,b}.
  \]
  After inverting \(\hbar\), this gives a filtered Fourier--Rees label
  bridge between nonconstant polynomial labels and the reduced
  principal parts \(\rho_{a,b}\), \(a+b>0\).  Over the Rees lattice
  before inverting \(\hbar\), the bridge records the degree-\(a+b\)
  shift by the explicit factor \(\hbar^{a+b}\).  The special fibre of
  the abstract Rees module keeps the labels
  ...
  This bridge is not an \(\mathfrak h\)-module bridge.  The
  ...
  Fourier--Rees label bridge cannot identify the polynomial descendant
  module with \(\mc P\) as a module over the same reduced Hamiltonian
  Lie algebra.
\end{thm}
...
\begin{proof}
  ...
  \[
    ...
    (-1)^{a+b}\hbar^{a+b}a!b!\rho_{a,b}.
  \]
  Removing the scalar label \(a=b=0\) gives the reduced label bridge
  ...
  Hamiltonian action is impossible; the actual Fourier comparison
  changes the polarization and supplies only the label-level bridge
  stated above.
\end{proof}

\begin{cor}[No flat same-action interpolation]
\label{cor:app-matlis-no-flat-same-action-interpolation}
  ...
  generic fibre.  Any honest bridge must therefore either change the
  Hamiltonian action by a Fourier twist, enlarge the boundary target to
  a distributional principal-part sector, or state a different
  deformation problem.
\end{cor}

\begin{proof}
  This is the local-nilpotence obstruction of
  Theorem~\ref{thm:app-matlis-no-polynomial-realization} after extension
...
\begin{rmk}[What the bridge proves]
\label{rmk:app-matlis-rees-bridge-status}
  The bridge above justifies the shared labels
  \(\Psi_{a,b}\leftrightarrow\rho_{a,b}\) as Fourier--Rees
  associated-graded bookkeeping, with the factorial and
  ...
  precisely where the apparent bridge breaks: Fourier sends the
