...
cochain-level derived-centre, principal-part, brane-line factorization,
degree-zero Hamiltonian Moyal, and scalar-anomaly statements.
...
weighted Tate completion or a nilpotent truncation. This
section closes the classical nilpotent-truncation route on a precisely specified
...
\ref{thm:open-closed-derived-center}. It is compatible with extension
...
\subsection*{Quantum status of the finite truncation}

\begin{thm}[Quantum status of the nilpotent truncation]
...
$$\begin{array}{ccc}
  ...
  Z^{P_0}_{\mathrm{red,Ham}}\bigl(\widehat{\mathbf S}(\mathfrak h_{\leq M,I})\bigr)\\
  \downarrow\pi_{M\to N} & & \downarrow\pi_{M\to N}\\
  C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g_{\leq N,I})
  & \xrightarrow{\ \Phi_{\leq N}\ } &
  Z^{P_0}_{\mathrm{red,Ham}}\bigl(\widehat{\mathbf S}(\mathfrak h_{\leq N,I})\bigr)
\end{array}$$
...
$$\Phi_{\mathfrak m^3}:=\varprojlim_N\Phi_{\leq N}:
  ...
  \xrightarrow{\ \sim\ }
  Z^{P_0}_{\mathrm{red,Ham}}\bigl(\widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak m^3)\bigr).$$
\end{prop}
...
The inverse limit
  ...
  classical cotangent lift on $\mathfrak m^3$, not on
...
weighted Tate completion, the universal cochain-level
...
nilpotent radicals (the route taken here) or replaces the Lie source
...
$\mathfrak m^3/\mathfrak m^{N+1}$ for $N\geq4$.  The inverse limit
$\Phi_{\mathfrak m^3}=\varprojlim_N\Phi_{\leq N}$ of
\ref{prop:colim-recovery} is the classical cotangent lift on the
full classical $\mathfrak m^3$ sector. What is lost: the low-degree sector
...
accommodate the semisimple $\mathfrak{sl}_2\subset H_2$. Thus the
nilpotent truncation
