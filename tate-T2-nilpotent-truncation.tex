...
\begin{prop}[Cubic inverse-limit survivor]
\label{prop:colim-recovery}
The classical truncated isomorphism $\Phi_{\leq N}$ of
\ref{thm:phi-trunc-classical} is compatible with the inverse
...
This is the maximal survivor of the nilpotent tower: the construction
recovers the complete cubic ideal \(\mathfrak m^3\), not the removed
linear and quadratic Hamiltonian sectors.
\end{prop}

\begin{proof}
Compatibility with the inverse system follows from the naturality of
...
relative or non-conilpotent model for the \(H_2\)-sector.
\end{thm}
...
\label{cor:full-maximal-ideal-universal-obstruction-class}
  ...
  \]
  Therefore a universal full-\(\mathfrak h\) Koszul theorem requires,
  ...
  relative or non-conilpotent treatment of the
...
\begin{proof}
  ...
  \(c\mapsto\theta,\ u\mapsto O\) is a completed dg \(P_0\)-algebra
  isomorphism.
...
\begin{thm}[Semisimple lower-central obstruction to Tate conilpotency]
...
\begin{proof}
  ...
  This contradicts the Tate-conilpotency condition used in
