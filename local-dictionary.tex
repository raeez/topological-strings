...
specific theorem supplies a comparison.  The only Fourier--Rees
comparison asserted here is
Theorem~\ref{thm:app-matlis-rees-fourier-bridge}: it matches
associated-graded labels after Fourier normalization through the
Fourier delta Rees lattice
\(\mathcal D_\hbar/(z_1,z_2)\), not through an
\(\mathfrak h\)-module identification and not through the full Cech
principal-part lattice before \(\hbar\) is inverted.
...
shadow and carries the corresponding polynomial action.\\
\hline
