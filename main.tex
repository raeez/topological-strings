...
\title{\texorpdfstring{Remarks on mixed topological-holomorphic strings\\
\large Closed Hamiltonian BF theory and reduced CE/PV central operations for a Dirac brane probe}
{Remarks on mixed topological-holomorphic strings:
Closed Hamiltonian BF theory and reduced CE/PV central operations for a Dirac brane probe}}
\date{April 28, 2026}
...
\makeatother
\hypersetup{
  pdftitle={Remarks on mixed topological-holomorphic strings},
  pdfauthor={Raeez Lorgat},
  pdfsubject={Closed Hamiltonian BF theory; formal local mixed holomorphic-topological model on R2 x C2; reduced Hamiltonian CE/PV central-operation model; componentwise quantum coefficient surface; finite-window non-scalar QME criterion; finite radial/Weyl certificates; scalar U(1) anomaly},
  pdfkeywords={BCOV source convention; Kodaira-Spencer analogue; Hamiltonian BF; Dirac brane probe; native holomorphic E2 factorization algebra; componentwise quantum coefficient surface; weighted BV kernel convergence; balanced scalar-contact zero projection; reduced weighted Moyal principal parts; finite-window non-scalar Costello QME criterion; theta3 source-face obstruction; Weyl/Moyal deformation quantization; radial/Weyl finite certificates; Matlis principal parts; Capelli triangular round-trip; scalar U(1) anomaly; formal symplectic disk}
}
\makeatletter
...
The object of study is a formal local constrained mechanical system. Let
\[
  X=\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}
    =\R_t\times\R_s\times\C^2_{z_1,z_2},
  \qquad
  L=\R_t\times\{s=z_1=z_2=0\}.
\]
The coordinate \(t\) is the brane direction and \(s\) is the topological
normal direction.  The holomorphic normal data is the formal Darboux disk
\[
  (\widehat{\C^2}_{0},\omega),\qquad \omega=dz_1\wedge dz_2 .
\]
If this disk is obtained from an ambient holomorphic symplectic surface,
the formal holomorphic Darboux theorem is the only input: the paper then
forgets the ambient surface and works on the completed normal disk.  The
defect is a \emph{point in \(\C^2\), dragged along \(\R_t\)}; neither
compact Calabi--Yau geometry nor a curve vertex algebra is part of the
local datum.

The two holomorphic normal coordinates $z_1,z_2$ describe the position of
the defect inside the symplectic disk. A stack of $N$ identical probes at
the defect represents each coordinate by an $N\times N$ matrix:
...
\end{equation}
When this action is extracted from the shifted open Chern--Simons
coefficient fields below, we first make the explicit linear
Dirac-normalization of those coefficient fields.  The sign convention is
therefore part of the local model, not an additional theorem.
This finite ghost-zero open model is the constrained mechanics of an
\(N\)-probe stack on the formal symplectic normal disk, with the matrix
moment-map equation as its first-class constraint.
Lemma~\ref{lem:dirac-probe-reduction} records the precise
...
The BV lift used here is the algebraic ghost-zero lift of
\eqref{eq:open-action-derived}; this lift is recorded in
...
compact threefold theory is not used as a transfer theorem for the present
formal-local \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) surface model.
...
First, the cotangent CE/PV identity is a cochain-level derived-centre
statement for the formal disk itself.  For finite cotangent Lie algebras
the generator map is
\[
  c^I\longmapsto\theta^I,\qquad u_I\longmapsto O_I,
\]
where \(c^I\) is dual to a Hamiltonian generator, \(u_I\) is dual to the
shifted cotangent generator, \(\theta^I\) is the Schouten vector-field
coordinate, and \(O_I\) is the boundary Hamiltonian function.  The
completed formal disk is obtained by compatible finite-coordinate tests.
Second, the promotion from cochains to an enveloping/Koszul-dual algebra
is a bar-cobar statement.  It is not asserted for the strict formal
Hamiltonian algebra without extra hypotheses. Whenever the coefficient
category supplies an admissible Tate, nilpotent, relative, or weighted
replacement \(\mathfrak g_{\mathrm{adm}}\), the Koszul dual is
\[
C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g_{\mathrm{adm}})^!
  \simeq \widehat{U(\mathfrak g_{\mathrm{adm}})}.
\]
For the strict object \(\mathfrak g\), failure of those hypotheses is the
universal obstruction datum recorded below:
Casimir convergence in the chosen Tate pair, the low-degree linear
translation term, the \(H_2\cong\mathfrak{sl}_2\) non-pronilpotent sector,
...
\end{equation}
This special fibre is only a PBW shadow.  The open one-\(\psi\)
descendants \(\Psi_{a,b}\) live in this associated-graded label system.
The deformation-level cotangent module is instead the continuous dual
\(P=\mathfrak h^\vee_{\mathrm{cont}}\), realized later by Matlis
principal parts \(\rho_{a,b}\).  No theorem below identifies
\(\Psi_{a,b}\) with \(\rho_{a,b}\) as \(\mathfrak h\)-modules or as
quantum boundary observables.
...
$\overline{\mathrm{Tr}(\phi_1^a\phi_2^b)}$, with \(a+b>0\) in the
reduced Hamiltonian label set.\\
$\Psi_{a,b}$ &
Primitive one-$\psi$ PBW special-fibre class, with \(a+b>0\) after
removing the scalar descendant line. It is not a Matlis principal part.\\
$\rho_{a,b}$ &
...
\subsection{Reduced Hamiltonian measurement and CE/PV central operations}
...
This computation exposes the obstruction that controls the paper.  The
full formal Hamiltonian algebra has no bracket-compatible finite Lie
quotient system obtained by cutting Taylor degree: linear Hamiltonians
lower degree, projected brackets fail Jacobi, and the diagonal
cotangent Casimir does not live in the strict product/direct-sum
endpoint.  Therefore the finite cotangent CE/PV theorem cannot be
transported to the disk by pretending that finite coordinate windows are
finite Lie algebras.

The survivor is different and sharper.  Finite coordinate projections
test coefficients only; they assemble to a completed coordinate dg
algebra isomorphism
\[
...
\]
A \(P_0\) comparison exists only after a bracket-admissible completion
\(B\) is chosen, and a Maurer--Cartan kernel exists only after the
diagonal tensor converges in the corresponding kernel-admissible tensor
product.  This is the reduced Hamiltonian central-operation model.  It
is not the unreduced factorization centre of the open factorization
algebra.

The map $f\mapsto J(f)$ is the \emph{moment-map shadow} of this reduced
formal CE/PV central-operation map. Witten's centrality slogan enters
here as a local algebraic criterion proved in the reduced Hamiltonian
model, not as a transfer theorem from BCOV.  Write
\(Z^{P_0}_{\mathrm{red,Ham}}(A;B)=B\) when \(B\subset\mathrm{PV}(A)\)
is the chosen bracket-admissible Poisson-CE model.  The boundary
observable $J(f)=\overline{\mathrm{Tr}\,f(\phi_1,\phi_2)}$ is the image,
under that identification, of the coordinate dual to the shifted
cotangent generator (\emph{not} of the coordinate dual to $f$ itself);
the closed Hamiltonian ghost becomes the Hamiltonian vector-field
operation on the brane.
...
\begin{thm}[Reduced Hamiltonian central-operation criterion, local \(P_0\) form]
  ...
  \]
  Thus the theorem-level datum proved here is the reduced Hamiltonian
  \(P_0\)-central-operation datum. A morphism to the unreduced derived
  \(P_0\)-factorization centre, including centrality homotopies against
  arbitrary open observables and principal-part cotangent currents, is
...
\begin{rmk}
  ...
  central direction. Scalar reduction kills the empty-trace scalar value
...
theorem.  It is a later admissibility target, organized by
Definition~\ref{defn:stable-ainfty-koszul-acceptance-datum} and proved
only inside the admissible Tate/stable category of
Theorem~\ref{thm:stable-ainfty-koszul-under-hypotheses}.  The local
...
reduction datum of
Statement~\ref{thm:lane-constructed-chiral-interface}.  If
...
\begin{rmk}
  ...
  fields are represented locally by Hamiltonians modulo constants.  This
  is not a truncation of threefold BCOV theory and no compact CY$_3$ BCOV
  theorem is used below.
\end{rmk}
...
\begin{rmk}
  ...
  manuscript proves the componentwise quantum coefficient surface:
  ...
  controlled by the radial/Weyl normal-form gate.  The current finite
  certificates close all non-edge bidegrees of total degree at most
  \(12\) and the balanced diagonal through \((6,6)\), while
...
Unless a statement is explicitly placed in the matched-conventions
firewall, every theorem below is a theorem about this completed
...
\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  ...
  topological-current extensions.  It does not mean a one-dimensional
  vertex algebra, an OPE mode algebra, compact-CY chiral homology, CoHA,
  or BKM data.  A one-dimensional VOA/OPE statement requires an
  additional theorem choosing a complex line or holomorphic defect, a
  transverse boundary condition or restriction functor, and the finite
  Laurent OPE expansion on that restricted object.
\end{defn}
...
\begin{proof}
  ...
  On an actual polydisk one keeps the compactly supported Dolbeault
  ...
  upgrades are the admissible consequences of
  Theorem~\ref{thm:formal-disk-completed-ce-pv} and
  Theorem~\ref{thm:reduced-ce-pv-central-operation}; no one-dimensional
  vertex/OPE structure is asserted.
\end{proof}

\begin{prop}[Bochner--Martinelli current data and finite-window obstruction for the native \(E_2\) lane]\label{prop:finite-window-bm-native-e2-transfer}
  Let
  ...
  \(i_Np_N\) is theorem-grade.  The component
...
\begin{proof}
  ...
  after the finite-current gate is crossed.  Away from collisions the
...
\begin{prop}[Native theorem package from the Darboux disk]\label{prop:native-darboux-theorem-package}
  For
  \[
  ...
  package consists of the following five native constructions.
  \begin{enumerate}
    ...
    \item After killing the scalar Hamiltonian and the empty trace, the
      derived-centre comparison is the coordinate CE/PV map
      \[
    ...
    \item The Bochner--Martinelli--Koppelman current limit, finite
      ...
      it requires first crossing this finite-current gate and then
      solving the six-entry all-window obstruction problem
      \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\).
  \end{enumerate}
  This package is internal to the formal local
  \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) model.  Compact
  Calabi--Yau theory, a curve VOA, CoHA, or BKM data can enter only
  through a separate matched-conventions theorem.
\end{prop}
...
\begin{proof}
  ...
  Proposition~\ref{prop:finite-window-bm-native-e2-transfer}.  The final sentence is
  the firewall already imposed by
  Definition~\ref{def:local-hamiltonian-chiral-factorization-algebra} and
...
\begin{rmk}[Reduced chiral-interface target]\label{rmk:constructed-chiral-interface-target}
  ...
  native Darboux package.  It must pass through the retained-mode
  \(\C\times\R\) interface, keeping the \(z_2\)-coefficient or the full
  two-index principal-part system.
  Its admission obstruction is
  \[
  ...
  \]
  measuring the retained-mode reduction, vertex/OPE package, Zhu
  multiplicativity, and comparison back to
  \(\mathcal F_{\mathrm{Ham}}^{\mathrm{hol}}\).
\end{rmk}

\begin{rmk}[External global-obstruction firewall]
  ...
  $\iota_{X_f}\omega=-df$. Thus any external attempt to globalize the
  Hamiltonian sector must first choose a sheaf of local Hamiltonians or
  kill the corresponding holomorphic de Rham obstruction. The formal disk
  used in this paper has no such obstruction by the holomorphic Poincare
  lemma; a general $X$ is not an input to the local theorem.
\end{rmk}

\begin{prop}[External formal-stalk Hamiltonian restriction]\label{prop:formal-local-global-restriction}
  ...
  This is an external comparison statement, not a hypothesis of
...
\begin{rmk}
  ...
  firewall for external holomorphic symplectic surfaces: it concerns
  restriction to a formal stalk only, not descent for factorization
  centres or quantum open-closed maps. If $H^1(X,\C)=0$, global
  ...
  classes. Such compact or global examples are comparison data only. A
  global cochain-level Poisson factorization-center map or a global quantum
  deformation-quantization statement requires additional descent, anomaly,
  and background-choice data and is not supplied by the local
...
Proposition~\ref{prop:skyscraper-ext}; the only Calabi-Yau datum used
here is the local residue/cyclic pairing on this two-dimensional Ext
...
source convention for the cyclic open BV form, not a compact
Calabi-Yau input; the local proof below is the finite Dirac/Koszul
matrix calculation.  For the local model at hand,
...
\begin{thm}[Loday--Quillen--Tsygan, external template]
...
\begin{rmk}[Same-rank deletion is not the LQT projection]
  ...
  is nonzero.  The theorem uses the stable block-sum Hopf cumulant
  ...
  CE complex, is the theorem-grade finite-rank bridge.  A same-rank
...
\subsection{Local theorem dependencies and analytic obstruction}\label{ssec:quantum-target}
...
bar-cobar/enveloping computation; and
the coordinate-free CE/PV closed-open synthesis of
Theorem~\ref{thm:coordinate-free-closed-open-synthesis}; the
componentwise quantum coefficient surface of
Theorem~\ref{thm:quantum-coefficient-surface}; and the first bracket
...
monomial coefficients are checked by finite exact arithmetic.  The
quantum coefficient theorem adds the proved weighted surface
\[
...
\]
This records weighted coefficient-kernel convergence and graphwise
boundedness, the balanced scalar-contact habitat with zero filtered
scalar projection, the reduced weighted Moyal principal-part current
target, and the radial/Weyl normal-form trace gate.  The finite scans
certify the balanced diagonal through \((6,6)\), every non-edge bidegree
of total degree at most \(13\), and the total-degree \(14\) cases
\[
  (3,11),(11,3),(4,10),(10,4),(5,9),(9,5);
\]
the first undecided total-degree \(14\) systems are
\((6,8),(8,6),(7,7)\).  The edge strips are no longer only
finite evidence: Proposition~\ref{prop:app-edge-pbw-telescoping} proves
the PBW telescoping primitive for every \((2,b)\) and \((a,2)\).  The
remaining uniform radial theorem is the interior dual-potential problem:
prove
\[
  \Omega^{\mathrm{rad}}_{a,b}
  =
  [(T_{a,b},E^+_{a,b})]\in\operatorname{coker}B_{a,b}
\]
vanishes for every \(a,b\geq3\), equivalently prove decorated PBW
Stokes \(D^\square_{a,b}=C^+_{a,b}\partial_2\).  A failure is precisely
a signed row \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
\(\lambda(E^+_{a,b})-\phi(T_{a,b})\ne0\).
...
finite-window counterterms and a curved bulk-to-defect kernel.  The
closed finite-window rows \(\alpha_{\mathrm{sc}}\) and \(\alpha_{11}\)
vanish; the \(\theta_3\) source-face row remains the first named
finite-row obstruction until its CE ancestor, local counterterm
primitive, or companion-face table is supplied.

For the binary centrality row, the proved statement is exactly the
zero-row statement in the reduced coefficient-current and minimal
full-equivariant habitat.  For
\[
...
\]
with zero Roos primitive class.  In a larger Costello local-functional
package the same assertion becomes the row problem \(A^Mh^M=-r^M\); it
is not a constructed centrality homotopy until the row values, scalar
gate, primitive columns, and transition matrices are supplied.

A closed-string Feynman-integral derivation of the Moyal coefficients is
a separate external analytic problem, not an input to the local theorem.
It requires the BV/gauge-fixing data of
Problem~\ref{prob:analytic-graph-realization}: an elliptic complex on a
chosen analytic avatar of \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\)
extending the formal jet model; boundary or defect conditions on $\C^2$; a
...
BV formalism supplies the general graph/RG formalism; Costello--Li prove
...
The local theorem has a classical core and a quantum coefficient
surface.  The classical core has three components. The open calculation
identifies the ghost-zero brane theory with the algebra of invariant
functions on commuting matrix pairs together with the first Koszul
descendants of the commuting-pair equation. The closed calculation
constructs the CE/PV coordinates of the Hamiltonian cotangent Lie
algebra and realizes its continuous cotangent module by Matlis principal
parts. The Hamiltonian boundary map is the local algebraic operation of
evaluating a Hamiltonian on the matrix-valued normal coordinates of the
brane; the polynomial one-$\psi$ descendants remain PBW special-fibre
classes, not principal-part currents.

Equivalently, the theorem constructs and compares the local objects named
in Definition~\ref{def:local-hamiltonian-chiral-factorization-algebra}:
...
Weyl/Moyal brane algebra, the defect principal-part current avatars, and
the Bochner--Martinelli--Koppelman current data, finite-window
obstruction gate, arity-two Matlis comparison, and one-pair analytic
pro-Matlis retract
\[
...
\]
of
Proposition~\ref{prop:finite-window-bm-native-e2-transfer}.  The theorem
does not construct a one-dimensional VOA or OPE algebra, nor a strict
all-arity BM transfer; those require the separate complex-line or
holomorphic-defect restriction theorem and, respectively, first the
vanishing or replacement of
\(\operatorname{Ob}^{\mathrm{BMK\mbox{-}curr}}_N\) and then the
six-entry all-window obstruction problem
\(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\).

The local theorem follows from the component theorems below. Its
reduced core is the coordinate CE/PV theorem and admissible Koszul
criterion of
...
after de Rham-current contraction, the scalar-reduced degree-zero
  Moyal/Weyl deformation, the componentwise quantum coefficient surface,
  and the U(1) scalar anomaly.  The full formal
  disk is a completed coordinate cochain theorem.  The \(P_0\) statement
  appears only after choosing an explicitly topologized
  bracket-admissible realization \(B\); boundary Hamiltonian uses require
  \(B\) to contain the relevant \(O_f\)'s and
  \(d_\pi^{\mathrm{coord}}O_f\)'s.  A kernel appears only after the
  diagonal converges in a specified kernel-admissible convolution dg Lie
  algebra.  The bar-cobar/enveloping upgrade is an admissibility
criterion:
...
are the obstruction data listed in the criterion.  The unreduced
...
\begin{rmk}[Finite-window status of the BM lane]\label{rmk:bm-lane-finite-window-status}
  Statement~\ref{thm:lane-native-holomorphic-e2-bm} is used below
  together with Proposition~\ref{prop:finite-window-bm-native-e2-transfer}.
  The Bochner--Martinelli--Koppelman obligation is discharged there as
  current-limit data, finite Matlis moments, and an arity-two coefficient
  comparison.  The finite-window differential identity is blocked, in the
  native full-current habitat, by
  \(\operatorname{Ob}^{\mathrm{BMK\mbox{-}curr}}_N\).  The strict
  all-window native \(E_2\) transfer remains downstream of that gate and
  then of the one-pair/pro-product split recorded by
  \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\).
  The positive all-window datum in this remark is only the one-pair
  analytic retract \(P_{\Pi}^{\mathrm{an}}=\tau P_{\mathrm{an}}\) with
  \(\pi_NP_{\Pi}^{\mathrm{an}}=p_N\), not a native
  factorization-current transfer.
\end{rmk}
    ...
    \item For the completed formal symplectic disk
      ...
      without these admissibility hypotheses.
	    \item If an admissible Tate, nilpotent, relative, or weighted
	      ...
	      \tau_{\mathrm{op}})\) of
	      Definition~\ref{defn:stable-ainfty-koszul-acceptance-datum} is
	      supplied and its obstruction complex is exact, then
	      \[
...
\begin{proof}
  ...
  Proposition~\ref{prop:cotangent-coordinate-boundary-hamiltonian}.  The
	  ...
	  lower-central Tate-pronilpotence hypothesis holds, while
	  Lemma~\ref{lem:filtered-cobar-qiso-criterion} and
	  Theorem~\ref{thm:stable-ainfty-koszul-under-hypotheses} supply the
	  additional open \(A_\infty\)-Koszul comparison only after the open-side
	  acceptance datum is given.  The obstruction
  terms in the fourth item are
  Theorem~\ref{thm:lane-strict-casimir-obstruction},
  Remark~\ref{rmk:colim-does-not-recover-full},
  Remark~\ref{rmk:conilpotency-essential}, and the obstruction clause of
...
\begin{thm}[Formal local Hamiltonian BF/Dirac-brane comparison]\label{thm:main-local}
  ...
  \[
    ...
    \mathfrak g=\mathfrak h\ltimes P[1].
  \]
  The manuscript constructs the native package of
  Proposition~\ref{prop:native-darboux-theorem-package}:
  \begin{enumerate}
    ...
    \item the coordinate derived-centre CE/PV map
      \[
    ...
    \item the Bochner--Martinelli--Koppelman current data, the
      ...
      comparison.
  \end{enumerate}
  Section~\ref{ssec:theorem-lanes} supplies the local proof dependencies:
  stable trace and one-\(\psi\) PBW shadow, coordinate CE/PV theorem with
  admissible Koszul criterion, Matlis principal parts, brane-line
  factorization currents, reduced principal-part defect currents,
  BMK current-limit data with finite-window obstruction gate and
  one-pair analytic pro-Matlis retract,
  scalar-reduced degree-zero
  Moyal/Weyl deformation, the componentwise quantum coefficient surface,
  and the U(1) scalar anomaly.

  Consequently the coordinate envelope
  \[
    C^\bullet_{\mathrm{CE},\mathrm{coord}}(\mathfrak g)
  \]
  ...
  Koszul-duality criterion are the formal-local spine of this coordinate
  identification.  This is a coordinate CE/PV and reduced-current
  statement.  It is not a morphism from closed BV states to the unreduced
  open factorization centre, and the principal-part current sector is the
  added reduced local-dual factor of
  Theorem~\ref{thm:reduced-principal-part-boundary-current}.  The cotangent
  direction is realized by the Matlis principal-part module; the
  polynomial one-\(\psi\) descendants give the PBW special-fibre labels
  and the primitive Koszul homology.  The proved quantum coefficient
  surface is
  the product
  \[
    \mathfrak S_{\hbar,w}(I)=
    ...
    \times T_{\mathrm{Ham},\hbar}
  \]
  of Theorem~\ref{thm:quantum-coefficient-surface}: weighted BV kernel
  ...
  controlled by the radial/Weyl normal-form gate.  The finite certificate
  range closes the balanced diagonal through \((6,6)\), all non-edge
  ...
  theorem is not a conditional slogan: it is the remaining
  dual-potential obstruction problem.  In the form of
  Proposition~\ref{prop:app-radial-dual-potential-obstruction}, this is
  the vanishing of
  \[
    \Omega^{\mathrm{rad}}_{a,b}
      =
  ...
  necklace cycle mechanism is proved; the missing theorem is the
  decorated PBW Stokes identity for
  \(D^\square_{a,b}=C^+_{a,b}\partial_2\).  The non-scalar
  Costello graph/QME construction has a theorem-grade finite-window
  criterion: it is the vanishing problem for
  \[
    [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]
    \in H^1(\ker\widehat\sigma_{\mathrm{sc}},d_{\mathrm{QME}})
  \]
  ...
  The analytic graph, regulator, unreduced cotangent, non-scalar QME,
  and cross-volume extensions are the obstruction and theorem data
  listed in
...
\begin{proof}
  Proposition~\ref{prop:native-darboux-theorem-package} fixes the Darboux
  habitat, the Dirac probe, the Hamiltonian BF coefficient algebra, the
  scalar quotient, and the PBW/deformation separation.
...
\begin{rmk}
  ...
  two-dimensional residue pairing fixed by \(dz_1dz_2\), not a global
  CY-to-chiral input. Global transfer would require the corresponding
  negative cyclic Calabi-Yau class, framing data, and descent; none of
  that is used in the proof below.
\end{rmk}
...
\begin{rmk}
  This is a source-comparison remark. The original BCOV analysis
  \cite{bcov} is written for Calabi-Yau threefolds; the higher-genus
  B-model on Calabi--Yau manifolds is taken up at the quantum level in
  \cite{costello-li-quantum-bcov}. On the formal holomorphic symplectic
  surface used here, the local cotangent theory is instead the BF theory
  for the dg Lie algebra of Hamiltonian vector fields. The holomorphic
  symplectic form supplies the Poisson bracket, while the local volume
  form \(dz_1dz_2\) supplies the divergence operator on polyvectors.
\end{rmk}
...
solution: an analytic avatar of the formal local model, not a compact
Calabi-Yau target and not a renaming of $X=M\times Y$. The
...
\begin{thm}[Admissible reduced Hamiltonian CE/PV central-operation map, cochain level]\label{thm:reduced-ce-pv-central-operation}
  Let
  \[
    \mathfrak h=\C[[z_1,z_2]]/\C\cdot1
  \]
...
\begin{prop}[Reduced coordinate coupling equations]\label{prop:coordinate-coupling-equations}
  ...
  QME data of Section~\ref{sec:open-obligations}.
\end{prop}
...
\begin{rmk}[Stratified Weiss/Cech--Roos descent obstruction]\label{rmk:weiss-ran-descent-obstruction}
  ...
  internal stratified lane the descent defect is the Cech cone
  \[
  ...
  \[
    ...
    \right).
  \]
  Source comparison with Costello--Gwilliam style global descent is a
  separate external matched-conventions gate, not an internal proof of the
  local factorization-center morphism.
\end{rmk}
...
The universal calculation has staged input.  The finite, coordinate, and
...
transported by this bookkeeping. The degreewise polynomial subalgebra
...
\begin{thm}[Support-local principal-part current \(P_0\) prefactorization datum]\label{thm:reduced-principal-part-boundary-current}
  ...
  Remark~\ref{rmk:unreduced-lift-status}.  We also write
...
\begin{thm}[Reduced Moyal commutator: radial input and obstruction gate]\label{thm:finite-n-reduced-moyal}
  ...
  The ordinary-trace comparison has the following exact status.  Assume
  the radial-parts input package of
  ...
  certificates cover the displayed interior windows, including the
  \(K_{4,4}\) lift and the non-edge total-degree checks recorded in
  Proposition~\ref{prop:app-free-obstruction-certificates} and the
  exact normal-form script.  For \(a,b\geq3\) in all degrees, the exact
  remaining problem is to construct the associated-graded two-colour
  necklace homotopy, or prove the dual-potential theorem: every signed
...
\begin{proof}
  ...
  The Harish-Chandra radial-parts package, whose source lineage is
  ...
  Hamiltonian reduction, and has the generator image normalization
	  ...
	  the mixed Weyl-trace image is kept as part of that input package,
  ...
  finite interior certificates are the free normal-diagram identities of
  ...
  These data are certificates and edge theorems; they do not supply the
...
\begin{cor}[Degree-zero quantum comparison and decorated radial obstruction]\label{cor:degree-zero-quantum-upgrade}
  Under the same radial-parts input package as
  ...
  Without the full radial image package, the formal Moyal coefficients
  ...
  certified interior windows.  Its all-interior extension is exactly the
...
\begin{rmk}[Degree-zero quantum Hamiltonian-sector status]\label{rmk:quantum-hamiltonian-upgrade}
  Under the radial image package,
  ...
  $$\hbar^{-1}[\partial_{\mathrm{bb},\hbar}(f),
      \partial_{\mathrm{bb},\hbar}(g)]_{\mathrm{conn,raw}}
    =\partial_{\mathrm{bb},\hbar}(\{f,g\}_\hbar).$$
  Equivalently, the degree-zero statement under that input is that the
  connected quantum brane algebra is the Weyl/Moyal deformation of the
  degree-zero Hamiltonian sector.  The associated graded of this proposed
  degree-zero quantum boundary comparison at $\hbar=0$ is the
  degree-zero part of Theorem~\ref{thm:main-local}.
  ...
  Combining these inputs, the closure of the degree-zero comparison on the
  ...
  The remaining interior construction problem is sharper than the old
  all-\(N\) radial slogan.  One must either prove the exact Weyl-trace
  image clause of Statement~\ref{stmt:app-radial-external-input}, or
  construct the equivalent quantum shear primitives
...
\begin{rmk}
  ...
  Remark~\ref{rmk:quantum-hamiltonian-upgrade}, because the polynomial
...
The graph problem is the analytic realization problem for the same
coefficient target. The algebra fixes the coefficients; a closed-open BV
theory must produce those
...
\begin{stmt}[Costello perturbative BV construction]\label{stmt:costello-bv-package}
...
\begin{stmt}[Quarantined source: Costello--Li flat open-closed BCOV theorem]\label{stmt:costello-li-flat-bcov}
  ...
  argument. This statement is recorded only as an exclusion check.
\end{stmt}
...
\begin{rmk}
  ...
  paper and are not inputs to Theorem~\ref{thm:main-local}. Here the brane
    ...
    \item after the first normalization, compute the three-propagator boundary
      ...
      -\partial_{z_2}\otimes\partial_{z_1}$;
	    ...
	    \item extend the bulk-to-defect kernel to the cotangent boundary-current
	      ...
	      Remark~\ref{rmk:unreduced-lift-status}. On the cotangent summand it
...
\label{thm:scalar-qme-alternatives}
  ...
  \]
  and, after any externally supplied scalar gate, by the image obstruction
  \[
...
\begin{proof}
  ...
  recorded in the scalar QME obligation lane; its differential is zero and
...
\begin{rmk}[Matched-conventions boundary]
  This local Hamiltonian BF sector supplies only the \(d=2\)
  formal-disk normalization for the Weyl/Moyal target.  It does not
  supply a compact Calabi-Yau target, a CoHA, a quintic sector, an
  HKQ/CDGP/GV/OSV input, an Abel--Jacobi normal function, a Vol III
  \(\Phi_d\)-datum, a \((\Sigma_{d-1},C)\)-specialization, an Igusa
  product lattice, a Borcherds Weyl chamber, or a BKM orientation
  character.  Compact Calabi-Yau, MNOP/chiral-volume,
  \(\kappa_{\mathrm{ch}}\), \(\kappa_{\mathrm{BKM}}\), compact
  CY\(_3\) BCOV anomaly, \(K3\times E\), CoHA, quintic, OSV/GV,
  Abel--Jacobi, Igusa, Borcherds, and BKM uses therefore require their
  own matched-conventions theorem.
...
\begin{thm}[Degree-zero Moyal target and decorated radial obstruction]\label{thm:phi-hbar-all-order}
  ...
  \begin{enumerate}
    ...
    \item The ordinary Weyl-trace image clause is equivalent, after the
      ...
      package, to the moment-ideal primitive
      ...
      Proposition~\ref{prop:app-free-trace-diagram-K44},
      Proposition~\ref{prop:app-free-obstruction-certificates}, and the
      current exact normal-form script supply finite interior
      certificates.  For all \(a,b\geq3\), the missing theorem is the
      associated-graded two-colour necklace homotopy, equivalently the
      dual-potential signed-row theorem above.
  \end{enumerate}

  If the radial image package of
  Statement~\ref{stmt:app-radial-external-input} is supplied, or if the
  equivalent quantum shear primitive package above is constructed
...
\begin{proof}
  ...
  statements cited there are exact free normal-diagram certificates; by
  Lemma~\ref{lem:app-free-trace-diagram-stable-injectivity} they are
  all-rank identities in their displayed windows.

  Under the radial image package, the displayed $\C[[\hbar]]$-linear
...
\begin{thm}[Quantum coefficient surface, componentwise]\label{thm:quantum-coefficient-surface}
  ...
  The componentwise quantum coefficient surface is the product
  ...
  \begin{enumerate}
    ...
    \item The reduced weighted quantum cotangent current brackets are
      ...
      classical principal-part current package is transported to the
      ...
      continuity.
    \item Under the radial image package, equivalently after solving the
      quantum shear primitive package of
  ...
  principal-part current target, and the radial-gated degree-zero Moyal
  coefficient comparison form a genuine componentwise theorem surface.
...
\begin{proof}
  ...
  \(B_{a,b}^*\)-rows as the failure certificates.  The product
  ...
  the displayed surface and no additional Costello graph/QME assertion.
\end{proof}
...
\begin{prob}[Non-scalar quantum \(P_0\)-operation obstruction complex]\label{prob:quantum-p0-operation-realization}
  ...
  Theorem~\ref{thm:quantum-coefficient-surface} is the non-scalar
  ...
  lifted through the scalar-contact filtration.  Thus the first
  obstruction is not the reduced non-scalar class, but the filtered
  lift of the scalar projection.  Choose a complete
  ...
  theorem obligation is the successive vanishing of these
  ...
  The balanced scalar-contact branch has a proved positive habitat:
  on
  ...
  full graph/counterterm habitat is a separate theorem obligation:
  ...
  problem.  Once the finite package supplies a scalar-zero basis
  ...
  Equivalently, the theorem obligation is to construct compatible
...
\begin{prop}[Finite-window acceptance gate for the first \(\theta_3\) row]\label{prop:theta-three-finite-window-acceptance-gate}
  In a larger finite-window graph package, let \(\Theta_3\) be a genuine
  ...
  finite-window package precisely by one of the following data, together
  ...
  for that source envelope only, not a universal no-go for marked-source,
...
\begin{proof}
  The current one-row package has no degree-zero term and has
  ...
  residual, so it is a left-cokernel certificate for the nonzero
  ...
  is theorem-grade only after the local functional, scalar-zero value,
  ...
  construct the analytic Costello graph package or any all-graph QME
  solution.
\end{proof}
...
\begin{rmk}
  ...
  Theorem~\ref{thm:phi-hbar-all-order}, under the radial image package
  of Statement~\ref{stmt:app-radial-external-input} or the equivalent
  quantum shear primitive package,
...
\begin{thm}[Costello first/third normalization recognition criterion]\label{thm:first-third-costello-normalizations}
  ...
  \begin{enumerate}
    ...
    \item any finite counterterm ambiguity that changes the first or third
      ...
      here.
  \end{enumerate}
  Equivalently, the Costello graph theorem at these orders holds precisely
...
\begin{proof}
  ...
  displayed defect tuple is a restatement of the hypotheses needed to make
  this reduced computation a Costello graph theorem: the first defect fixes
  ...
  reduced-kernel, and \(\theta_3\) graph-row gates developed in
  Problem~\ref{prob:quantum-p0-operation-realization}.
\end{proof}
    ...
    \item identify the connected three-propagator brane-defect ribbon graphs
      whose tensor contraction is $P^3(f,g)$;
	    ...
	    \item verify that the same observable product, counterterm subtraction,
	      ...
	      Remark~\ref{rmk:unreduced-lift-status}. In particular the degree-zero
...
\section{Matched-conventions firewall}\label{sec:matched-conventions-firewall}

\input{tate-P3-universality}

\subsection{Cross-volume matched-conventions theorem}\label{ssec:cross-volume-firewall}
...
brane probe.  Compact Calabi-Yau, CoHA, quintic, HKQ/CDGP/GV/OSV,
Abel--Jacobi, MNOP/chiral-volume, Borcherds/BKM, and global
...
a \(K3\times E\) specialisation, CoHA, quintic, OSV/GV, Abel--Jacobi, or
a BKM denominator identity.
