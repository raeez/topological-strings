...
\title{\texorpdfstring{Formal Local Hamiltonian BF Theory and Reduced CE/PV Central Operations for a Dirac Brane Probe\\
\large Formal local mixed holomorphic-topological model on \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\)}
{Formal Local Hamiltonian BF Theory and Reduced CE/PV Central Operations for a Dirac Brane Probe:
Formal local mixed holomorphic-topological model on R2 top x C2 hol}}
\date{April 30, 2026}
...
\makeatother
\hypersetup{
  pdftitle={Formal Local Hamiltonian BF Theory and Reduced CE/PV Central Operations for a Dirac Brane Probe},
...
There are two structural statements here, and they are kept separate.
First, the cotangent CE/PV identity is a cochain-level reduced
central-operation statement for the formal disk itself.  For finite
cotangent Lie algebras
...
admissibility obstruction datum recorded below:
...
\begin{rmk}
  ...
  certificates close the balanced diagonal through \((6,6)\), all
...
\begin{prop}[Native theorem package from the Darboux disk]\label{prop:native-darboux-theorem-package}
  ...
  through a separate comparison theorem with its own hypotheses.
\end{prop}
...
\begin{rmk}[Reduced chiral-interface target]\label{rmk:constructed-chiral-interface-target}
  ...
  target, not part of the native Darboux package.  It must pass through a
...
Feynman-integral derivation of the same coefficients is a separate
analytic problem.  The proof of Theorem~\ref{thm:main-local} uses the
formal local algebra alone and is independent of those analytic choices.
...
\begin{thm}[Formal local Hamiltonian BF/Dirac-brane comparison]\label{thm:main-local}
  ...
  \[
    ...
    \mathfrak g=\mathfrak h\ltimes P[1].
  \]
  The manuscript constructs the following native package:
  \begin{enumerate}
  ...
  The analytic graph, regulator, unreduced cotangent, and non-scalar QME
  extensions are not part of this theorem unless their additional
  coefficient, kernel, and QME data are supplied.
\end{thm}
...
\begin{rmk}[Stratified Weiss/Cech--Roos descent obstruction]\label{rmk:weiss-ran-descent-obstruction}
  ...
  separate external comparison problem, not an internal proof of the
...
\begin{rmk}
  ...
  $\Psi_{k,l}$. Corollary~\ref{cor:descendant-coadjoint-difference} shows that
	  their open Hamiltonian action is the PBW special-fibre Hamiltonian action, while the
...
\begin{rmk}
  The two source statements supply the general renormalized BV framework and
