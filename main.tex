...
\title{\texorpdfstring{Closed Hamiltonian BF Theory and Reduced CE/PV Central Operations for a Dirac Brane Probe\\
...
\makeatother
\hypersetup{
  pdftitle={Closed Hamiltonian BF Theory and Reduced CE/PV Central Operations for a Dirac Brane Probe},
  pdfauthor={Raeez Lorgat},
  pdfsubject={Closed Hamiltonian BF theory; formal local mixed holomorphic-topological model on R^2 x C^2; reduced Hamiltonian CE/PV central-operation model; factorization-centre obstruction datum; degree-zero Weyl/Moyal comparison; scalar U(1) anomaly},
  pdfkeywords={BCOV source convention; Kodaira-Spencer analogue; Hamiltonian BF; Dirac brane probe; factorization-centre obstruction datum; locally constant factorization algebra; reduced Hamiltonian CE/PV central operations; Weyl/Moyal deformation quantization; Matlis principal parts; Capelli triangular round-trip; queer Lie superalgebra; scalar U(1) anomaly; formal symplectic disk}
}
\makeatletter
...
It is not asserted for the strict formal Hamiltonian algebra without
extra hypotheses. Whenever the coefficient category supplies an
admissible Tate, nilpotent, relative, or weighted replacement
\(\mathfrak g_{\mathrm{adm}}\), the Koszul dual is
\[
...
\begin{rmk}
  ...
  and records a conditional degree-zero Hamiltonian central-operation
  comparison; the full renormalized closed-open BV datum remains the
  analytic graph-realization problem of
...
The local theorem has three components. The open calculation identifies the
...
\begin{thm}[Formal local Hamiltonian BF/Dirac-brane comparison]\label{thm:main-local}
  ...
  Dirac brane probe, stable trace and one-$\psi$ PBW shadow,
	  coordinate CE/PV theorem with admissible Koszul criterion, Matlis principal parts,
  ...
  the scalar trace on the brane side, the coordinate envelope
  $C^\bullet_{\mathrm{CE},\mathrm{coord}}
    (\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1])$
  gives the CE/PV Hamiltonian central-operation model attached
  to the stable open Hamiltonian brane algebra.  The finite
	  cotangent identity, coordinate formal-disk reconstruction, and
		  admissible Tate Koszul-duality criterion are the formal-local spine of
  this coordinate identification.  This is a coordinate CE/PV and
  reduced-current statement.  It is not a morphism from closed BV states
  to the unreduced open factorization centre, and the principal-part
  current sector is the added reduced local-dual factor of
  ...
  primitive Koszul homology.  The unconditional quantum datum is the
  formal Weyl/Moyal coefficient algebra and the scalar anomaly class
  $[\bar c]$; its finite-\(N\) and stable connected trace realization is
  conditional on the radial-parts input of
  Statement~\ref{stmt:app-radial-external-input}.  The one-antifield
  principal-part quantum target is governed by the obstruction complex of
...
\begin{proof}
  The listed component theorems give the proof dependencies and local
  ...
  Moyal/Weyl, and scalar-anomaly statements gives the displayed reduced
  Hamiltonian CE/PV central-operation statement.
\end{proof}
...
\begin{constr}[Reduced line-defect current kernel]\label{constr:reduced-line-defect-current-kernel}
  ...
  brane-defect kernel is not constructed here: the Ward identity, anomaly cancellation,
...
\begin{prob}[Unreduced Tate-coefficient cotangent lift datum]\label{prob:formal-factorization-center}
  ...
  BCOV/HCS theorem, which does not transfer to the present mixed defect geometry;
  weighted
...
\begin{rmk}[Weiss/Ran descent obstruction complex]\label{rmk:weiss-ran-descent-obstruction}
  A global factorization-center morphism is not obtained by choosing Darboux coordinates at
...
\begin{rmk}[Degree-zero quantum Hamiltonian-sector status]\label{rmk:quantum-hamiltonian-upgrade}
  ...
  $$\hbar^{-1}[\partial_{\mathrm{bb},\hbar}(f),
      \partial_{\mathrm{bb},\hbar}(g)]_{\mathrm{conn,raw}}
    =\partial_{\mathrm{bb},\hbar}(\{f,g\}_\hbar).$$
  Equivalently, the degree-zero statement is that the connected quantum brane
  ...
  The remaining unproved datum is the all-\(N\) radial-parts normalization
  packaged as Statement~\ref{stmt:app-radial-external-input}.
\end{rmk}
...
\label{thm:scalar-qme-alternatives}
  ...
  The following branch statements are proved, or conditional in the exact
  sense stated:
  \begin{enumerate}
    ...
    \item In the original scalar-reduced ordinary \(\lie{gl}_N\) model, a
      closed-exchange or brane-defect contribution \(W_{\mathrm{ext}}\)
      can cancel the scalar symbol only if its leading scalar class obeys
      \[
        [w_{\mathrm{ext}}]=-\hbar N[\bar c]
        \qquad\text{in }H^2_{\mathrm{Lie}}(\bar A;\C)[[\hbar]].
      \]
      If such \(W_{\mathrm{ext}}\) and a local counterterm \(C\) are
      explicitly constructed with
      \[
        \operatorname{Ob}_{\mathrm{sc}}+W_{\mathrm{ext}}
          +d_{\mathrm{QME}}C=0,
      \]
      then the scalar QME component is cancelled.
  \end{enumerate}
...
\begin{proof}
  ...
  original reduced obstruction complex.  The balanced-supertrace branch is
  Theorem~\ref{thm:app-balanced-supertrace-qme-cancellation}.  The final
  necessary condition and conditional cancellation statement are
  Theorem~\ref{thm:app-ordinary-glN-scalar-obstruction-criterion}.  These
  results prove the listed branches and no more.
\end{proof}
...
\begin{rmk}[Matched-conventions boundary]
  ...
  supply a compact Calabi-Yau target, a Vol III \(\Phi_d\)-datum, a
  \((\Sigma_{d-1},C)\)-specialization, an Igusa product lattice, a
  Borcherds Weyl chamber, or a BKM orientation character.  Compact
  Calabi-Yau, MNOP/chiral-volume, \(\kappa_{\mathrm{ch}}\),
  \(\kappa_{\mathrm{BKM}}\), compact CY\(_3\) BCOV anomaly,
  \(K3\times E\), Igusa, Borcherds, and BKM uses therefore require
  their own matched-conventions theorem.
...
component of the quantum \(P_0\)-operation comparison. It is not a
construction of the descendant sector and it is not the Costello graph
realization.

\begin{thm}[Conditional all-order degree-zero quantum Hamiltonian comparison]\label{thm:phi-hbar-all-order}
  Assume the external radial-parts input of
  ...
  here.
  Equivalently, this theorem proves the Moyal identity for the ordinary
  Weyl traces only relative to the all-\(N\) radial image normalization.
  Without that input it proves the formal Moyal target and leaves the
  ordinary-trace comparison as the radial-normalization problem.
...
\begin{prob}[Quantum \(P_0\)-operation realization datum]\label{prob:quantum-p0-operation-realization}
  The desired quantum \(P_0\)-operation comparison
  $$\Phi_\hbar^{\mathrm{cand}}\colon
    ...
    \bigr)[[\hbar]]
    \longrightarrow
    \mathcal P^{\mathrm{des}}_{0,\hbar,\mathrm{fact}}
      \bigl(\mathrm{Obs}^\hbar_{\mathrm{open}}\bigr)$$
  is not constructed here.  The target symbol denotes only a desired
  quantum \(P_0\)-operation object; no associative or \(E_1\)
  center is asserted.  The radial-parts input of
...
\begin{thm}[Conditional first/third normalization test for a supplied Costello specialization]\label{thm:first-third-costello-normalizations}
...
brane probe.  Compact Calabi-Yau, MNOP/chiral-volume, Borcherds/BKM, and
global twisted-M-theory consequences are not consequences of this local
theorem.
