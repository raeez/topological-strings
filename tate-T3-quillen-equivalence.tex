...
\begin{stmt}[Admissible Tate model envelope]\label{stmt:tate-model-envelope}
  ...
  limits with surjective transition maps are exact.  Assume this
  envelope satisfies the transfer and smallness hypotheses for the
  bar-cobar adjunction, the monoid axiom for completed tensor products,
  and compatibility of \(\widehat\otimes\) with the finite-window model
  structures.  All Quillen statements below are relative to this envelope.
\end{stmt}

\begin{lem}[Mittag-Leffler dictionary for closed Tate windows]
\label{lem:tate-mittag-leffler-dictionary}
  Let \(\{V_{\leq M}\}_{M\geq1}\) and
  \(\{W_{\leq M}\}_{M\geq1}\) be inverse systems of
  finite-dimensional complexes with surjective transition maps.  Put
  \(V=\varprojlim_MV_{\leq M}\) and \(W=\varprojlim_MW_{\leq M}\).
  In the admissible envelope:
...
\begin{proof}
  ...
  Statement~\ref{stmt:tate-model-envelope} is defined by the same
  finite-window inverse system, so item (iii) is tautological inside
  the envelope.  Item (iv) is the definition of weak equivalence in
...
\subsubsection{Direct universal recognition}

\begin{defn}[Universal open-side twisting cochain]
\label{defn:universal-open-side-twisting-cochain}
  ...
  reduced coproducts \(\Delta^{(r)}\).  A universal open-side twisting
  cochain is a continuous degree-one map
  \[
...
\begin{defn}[Universal Koszul obstruction complex]
\label{defn:universal-koszul-obstruction-complex}
  ...
  window.  The universal obstruction datum is
  \[
...
\begin{thm}[Universal filtered-cobar acceptance criterion]
\label{thm:universal-filtered-cobar-recognition}
  ...
  finite-dimensional associated graded pieces in every total degree, and
  assume continuous dualization is exact on the finite-window towers.
...
\begin{proof}
  ...
  The equation \(\mathcal O_{\mathrm{MC}}(\tau)=0\) is the
