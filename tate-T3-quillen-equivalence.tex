...
\label{defn:closed-window-obstruction-terms}
  ...
  \]
  The closed window system is \emph{CE/PV-admissible} if these three
  obstruction terms vanish for all \(M\geq N\).
\end{defn}

\begin{lem}[Closed CE/PV identity passes through finite windows]
\label{lem:closed-ce-pv-window-limit}
  ...
  CE/PV-admissible and each finite window has the finite cotangent
  CE/PV identity
  \[
    \Phi_M\colon
  ...
  \]
  Then the \(\Phi_M\) form a strict inverse system and induce an
...
\begin{proof}
  ...
  Vanishing of \(\mathfrak o^{\mathrm{Cas}}_{M,N}\) says the finite
  \(P_0\)-brackets and BV coefficient pairings are compatible under
  transition.  Therefore the squares with \(\Phi_M\) and \(\Phi_N\)
  commute.

  Taking the inverse limit gives the displayed completed complexes.
  Lemma~\ref{lem:tate-mittag-leffler-dictionary} applies because the
  transition maps are surjective and finite-dimensional on every
  window.  Since every \(\Phi_M\) is a quasi-isomorphism and the
  associated graded map is the same finite generator substitution, the
  inverse-limit map is an admissible filtered quasi-isomorphism.
\end{proof}
...
\label{defn:universal-open-side-twisting-cochain}
  Fix a candidate formal-local Koszul datum
  \[
...
\begin{proof}
  ...
  finite-window compatibility of the CE differential, the Schouten
  differential, and the finite \(P_0\)-pairings; this is exactly
  Lemma~\ref{lem:closed-ce-pv-window-limit} and
  Theorem~\ref{thm:universal-formal-local-ce-pv-recognition}.  The
  vanishing of \(\mathcal O_{\mathrm{top}}\) is the Milnor condition
  identifying cohomology of the completed CE/PV objects with the inverse
  limit of finite-window cohomology.
...
\begin{lem}[Closed-side Tate/Koszul formalism package]
\label{lem:closed-tate-koszul-formalism}
  ...
  CE/PV-admissible in the sense of
  ...
  \[
    ...
    \mathrm{PV}\bigl(\widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h)\bigr)
  \]
  is the finite-window inverse-limit CE/PV identity of
  Lemma~\ref{lem:closed-ce-pv-window-limit}, and
  \[
...
\begin{proof}
  The first output is exactly
  Lemma~\ref{lem:closed-ce-pv-window-limit}.  The obstruction terms
  \(\mathfrak o^{\mathrm{br}}\), \(\mathfrak o^{\mathrm{coad}}\), and
  \(\mathfrak o^{\mathrm{Cas}}\) are the only possible failures of
  transition compatibility for the CE differential, the Schouten
  differential, and the \(P_0\)-pairing on finite windows; their
  vanishing makes \(\Phi=\varprojlim_M\Phi_M\) well-defined.
...
\label{thm:open-closed-derived-center-derived}
  ...
  $$\Phi\colon C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g_{\mathrm{adm}})
    ...
    \widehat S_{\mathrm{Tate}}(\mathfrak h_{\mathrm{adm}})\bigr)$$
      is the finite-window inverse-limit CE/PV quasi-isomorphism of
      Lemma~\ref{lem:closed-ce-pv-window-limit};
