...
\label{lem:app-sign-current-source}
  ...
  \(B\in\mathcal D'_c(I)\), multiplication by a test function is the
  ordinary distribution product \(aB\), and
  \[
...
\begin{proof}
  ...
  brane line; the Hamiltonian factor supplies \(f\).  Pairing this source
...
\label{cor:app-sign-universal-dual-normalization}
  ...
  principal-part module.  With it fixed, the polynomial descendant formulas
  \(z_1\cdot\Psi_{a,b}=b\Psi_{a,b-1}\) and
  \(z_2\cdot\Psi_{a,b}=-a\Psi_{a-1,b}\) cannot be made into the same
...
\begin{proof}
  ...
  basis vectors can turn a locally nilpotent lowering action into this
  non-locally-nilpotent raising action; the obstruction is structural, not a
  convention error.
\end{proof}
